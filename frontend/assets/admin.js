...
const Admin = {
  ...
  view_stations() {
    ...
    wrap.appendChild(el('p', 'hint',
      ...
      + 'бармена или кухаря — тогда в смене остаётся имя, а барменов за вечер '
      + 'бывает двое. PIN станции ниже — запасной, на случай забытого своего: '
      + 'он открывает смену без имени.'));
    ...
    if (this.data.shifts.length) {
      ...
      wrap.appendChild(this.table(
        ['Станция', 'Открыл', 'Открыта', 'Закрыл', 'Закрыта', 'Марок'],
