...
def main() -> None:
    with sync_playwright() as pw:
        ...
        # Планшет стоит на полке месяцами и сам не перезагружается: без
        # кнопки поправка доезжает только после того, как его переставят.
        check("на планшете есть чем обновиться", bar.locator("#fresh").count() == 1)
        bar.locator("#fresh").click()
        bar.wait_for_timeout(2500)
        check("после обновления смена не потерялась",
