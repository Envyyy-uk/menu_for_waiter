...
def test_stock_is_for_owner_and_admin_only(client, hall):
    ...
    assert "stock.view" not in login(client, "4444")["permissions"]   # менеджер
    assert "stock.view" in login(client, "1234")["permissions"]       # владелец
...
def test_owner_can_hand_out_any_role(client, hall):
    login(client, "1234")
    made = client.post(
        "/api/admin/users", json={"name": "Второй владелец", "role": "owner", "pin": "9090"}
    )
    assert made.status_code == 201
    assert client.post("/api/auth/pin", json={"pin": "9090"}).json()["role"] == "owner"


def test_admin_cannot_make_an_owner(client, hall):
    login(client, "1234")
    admin = client.post(
        "/api/admin/users", json={"name": "Админ", "role": "admin", "pin": "7070"}
    ).json()
